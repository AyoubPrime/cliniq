...
export default function AdminPage() {
  ...
  const handleSubmit = async () => {
    ...
    const aliases = [form.alias1, form.alias2, form.alias3].filter(a => a.trim() !== '')
    const red_flags = [form.red_flag1, form.red_flag2, form.red_flag3].filter(r => r.trim() !== '')
    const management = [form.management1, form.management2, form.management3, form.management4].filter(m => m.trim() !== '')
    const common_mistakes = [form.mistake1, form.mistake2].filter(m => m.trim() !== '')
    const differentials = [
    ...
    const diagnostic_approach = [1,2,3,4,5]
      .map(n => ({
        step: n,
    ...
    if (error) {
      ...
      setForm({
        ...
        age: '', age_unit: 'ans', sex: 'F', setting: 'Urgences',
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <span className="text-xl font-semibold text-gray-900">Clin</span>
            <span className="text-xl font-semibold text-blue-600">IQ</span>
            <p className="text-sm text-gray-400 mt-0.5">Nouveau cas</p>
          </div>
        </div>
            ...
            {[1,2,3,4,5].map(n => (
              <div key={n} className="mb-3 p-3 bg-gray-50 rounded-xl">
                ...
                <input
                  ...
                  placeholder={`Titre de l'étape ${n}...`}
                  value={form[`approach${n}_title` as keyof typeof form] as string}
                  onChange={e => update(`approach${n}_title`, e.target.value)}
                />
                <textarea
                  className={inputClass}
                  rows={2}
                  placeholder="Détail clinique..."
                  value={form[`approach${n}_detail` as keyof typeof form] as string}
                  onChange={e => update(`approach${n}_detail`, e.target.value)}
                />
              </div>
            ))}
          </div>
