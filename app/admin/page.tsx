...
const emptyForm = {
  ...
  approach1_title: '', approach1_detail: '',
  approach2_title: '', approach2_detail: '',
    ...
    <div>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-4 gap-2">
          {[['TA','bp','120/80'],['FC','hr','80'],['Temp °C','temp','37.5'],['SpO2 %','spo2','98']].map(([lbl, field, ph]) => (
            <div key={field}>
      ...
      <div className={sectionClass}>
        ...
        {[1,2,3,4,5,6].map(n => (
          <div key={n} className="mb-2">
            ...
            <input className={inputClass} value={form[`clue${n}` as keyof FormType] as string} onChange={e => update(`clue${n}`, e.target.value)} />
          </div>
        ))}
      </div>
      ...
      <div className={sectionClass}>
        ...
        <div className="mb-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[1,2,3].map(n => (
              <input key={n} className={inputClass} placeholder={`Alias ${n}`} value={form[`alias${n}` as keyof FormType] as string} onChange={e => update(`alias${n}`, e.target.value)} />
            ))}
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-3">
      ...
      <div className={sectionClass}>
        ...
        <div className="mb-3">
          ...
          {[1,2,3].map(n => (
            <input key={n} className={inputClass + ' mb-2'} placeholder={`Signe d'alarme ${n}...`} value={form[`red_flag${n}` as keyof FormType] as string} onChange={e => update(`red_flag${n}`, e.target.value)} />
          ))}
        </div>
        <div className="mb-3">
          <label className={labelClass}>Prise en charge initiale</label>
          {[1,2,3,4].map(n => (
            <input key={n} className={inputClass + ' mb-2'} placeholder={`Étape ${n}...`} value={form[`management${n}` as keyof FormType] as string} onChange={e => update(`management${n}`, e.target.value)} />
          ))}
        </div>
        <div className="mb-3">
          <label className={labelClass}>Erreurs classiques</label>
          {[1,2].map(n => (
            <input key={n} className={inputClass + ' mb-2'} placeholder={`Erreur ${n}...`} value={form[`mistake${n}` as keyof FormType] as string} onChange={e => update(`mistake${n}`, e.target.value)} />
          ))}
        </div>
        <div className="mb-3">
          ...
          {[1,2,3].map(n => (
            <div key={n} className="mb-3 p-3 bg-gray-50 rounded-xl">
              ...
              <input className={inputClass + ' mb-2'} placeholder="Diagnostic..." value={form[`diff${n}_diagnosis` as keyof FormType] as string} onChange={e => update(`diff${n}_diagnosis`, e.target.value)} />
              <select className={inputClass + ' mb-2'} value={form[`diff${n}_proximity` as keyof FormType] as string} onChange={e => update(`diff${n}_proximity`, e.target.value)}>
                <option value="proche">Proche</option>
                <option value="faux">Faux</option>
              </select>
              <input className={inputClass} placeholder="Pourquoi écarté..." value={form[`diff${n}_distinction` as keyof FormType] as string} onChange={e => update(`diff${n}_distinction`, e.target.value)} />
            </div>
          ))}
        </div>
        <div>
          ...
          {[1,2,3,4,5].map(n => (
            <div key={n} className="mb-3 p-3 bg-gray-50 rounded-xl">
              ...
              <input className={inputClass + ' mb-2'} placeholder={`Titre de l'étape ${n}...`} value={form[`approach${n}_title` as keyof FormType] as string} onChange={e => update(`approach${n}_title`, e.target.value)} />
              <textarea className={inputClass} rows={2} placeholder="Détail clinique..." value={form[`approach${n}_detail` as keyof FormType] as string} onChange={e => update(`approach${n}_detail`, e.target.value)} />
            </div>
          ))}
        </div>
      </div>
...
useEffect(() => {
  ...
  const loadCase = async (id: string) => {
    ...
    setForm({
      id: data.id, title: data.title || '', specialty: data.specialty || '',
      difficulty: data.difficulty || 2, publish_date: data.publish_date || '',
      age: data.age?.toString() || '', age_unit: data.age_unit || 'ans',
      sex: data.sex || 'F', setting: data.setting || 'Urgences',
      ...
      diff1_diagnosis: getDiff(1,'diagnosis'), diff1_proximity: getDiff(1,'proximity') || 'proche', diff1_distinction: getDiff(1,'distinction'),
      diff2_diagnosis: getDiff(2,'diagnosis'), diff2_proximity: getDiff(2,'proximity') || 'faux', diff2_distinction: getDiff(2,'distinction'),
      diff3_diagnosis: getDiff(3,'diagnosis'), diff3_proximity: getDiff(3,'proximity') || 'proche', diff3_distinction: getDiff(3,'distinction'),
      approach1_title: getApproach(1,'title'), approach1_detail: getApproach(1,'detail'),
      approach2_title: getApproach(2,'title'), approach2_detail: getApproach(2,'detail'),
      approach3_title: getApproach(3,'title'), approach3_detail: getApproach(3,'detail'),
      approach4_title: getApproach(4,'title'), approach4_detail: getApproach(4,'detail'),
      approach5_title: getApproach(5,'title'), approach5_detail: getApproach(5,'detail'),
    })
    setEditingId(id)
  ...
  useEffect(() => {
    if (authenticated && (tab === 'edit' || tab === 'view')) loadCases()
  }, [authenticated, tab])
  ...
  const buildPayload = () => ({
    ...
    diagnostic_approach: [1,2,3,4,5]
      .map(n => ({
        step: n,
        title: form[`approach${n}_title` as keyof FormType] as string,
        detail: form[`approach${n}_detail` as keyof FormType] as string,
      }))
        ...
        {tab === 'edit' && (
          <div>
            {!editingId ? (
              ...
              <div>
                <button onClick={() => { setEditingId(null); setForm(emptyForm); setConfirmDelete(false) }} className="text-sm text-gray-400 mb-4">
                  ← Retour
                </button>
                <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-4">
                  <p className="text-xs text-blue-600">Modification de : <strong>{editingId}</strong></p>
                </div>
                <CaseForm form={form} update={update} isEdit={true} />
                {message && (
                  <div className={`p-4 rounded-xl mb-4 text-sm font-medium ${message.includes('Erreur') ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                    {message}
                  </div>
